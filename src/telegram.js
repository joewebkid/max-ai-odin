...
import { AudioInputError, extractTelegramAudioInput } from './audio-input.js';
...
bot.command('start', async (ctx) => {
  ...
  await sendModeMenu(
    ctx,
    activeBackend,
    'Бот готов. Отправьте текст или короткое голосовое, и я передам запрос через выбранный режим. Доступны Free и Chat GPT. Команды: /help, /mode, /tariff и /reset.',
  );
});
...
bot.on('message', async (ctx) => {
  ...
  try {
    audioInput = await extractTelegramAudioInput(ctx, {
      botToken: config.telegramBotToken,
      proxyUrl: config.telegramProxyUrl,
